...
export class RecipesService {
  ...
  recipes$ = this.http.get<Recipe[]>(this.config.apiEndpoint + "/recipes", {
  ...
  getRecipes() {
    ...
    return this.http.get<Recipe[]>(this.config.apiEndpoint + "/recipes", {
      headers: new HttpHeaders({
  ...
  getRecipeById(recipeId: number) {
    return this.http.get<Recipe>(this.config.apiEndpoint + "/recipes" + "/" + recipeId);
    // return this.http.get<Recipe>("http://localhost:8080/recipes/1");
  }

  addRecipe(recipe: Recipe, userId : number) {
    return this.http.post<Recipe[]>(this.config.apiEndpoint + "/recipes" + "/add/" + userId, recipe);
  }

  updateRecipe(recipe: Recipe, userId :number) {
    return this.http.put<Recipe[]>(this.config.apiEndpoint + "/recipes/" + userId, recipe);
  }

  deleteRecipe(recipeId:number,userId:number) {
    console.log(recipeId +" "+userId);
    return this.http.delete<Recipe[]>(this.config.apiEndpoint + "/recipes" + "/" + userId + "/" + recipeId);
  }
